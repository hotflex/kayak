...
module.exports = (dependencyRoot, pullData, loop, isSubDependency) => {
	...
	let lock = {
		...
		source: `git+${processUrl(repo)}${branch ? "#" + branch : ""}`,
		commit: commit,
	}
	...
	if (rotriever.dependencies) {
		...
		for (childD in rotriever.dependencies) {
			const subDependency = loop(childD, rotriever.dependencies[childD], finalContainer, true)
			if (subDependency) {
				lock.dependencies.push(
					`${childD} ${subDependency.name} ${
						subDependency.version
					} ${subDependency.commit.substring(0, 8)} ${subDependency.source}`
				)
			}
		}
	}

...
module.exports = (data, dependencyRoot, index) => {
	...
	for (d in data.dependencies) {
		 ...
		 */
		const thisTemp = path.join(
			dependencyRoot,
			`.__${sanitizeFilename(d, sanitizeFilenameOptions)}_001`
		)
		...
		const sanitizedPackage = sanitizeFilename(
			rotriever.package.name,
			sanitizeFilenameOptions
		).toLowerCase()
		const sanitizedFinal = sanitizeFilename(d, sanitizeFilenameOptions)

		const thisFinalContainer = path.join(index, sanitizedPackage)
		const thisFinal = path.join(thisFinalContainer, sanitizedFinal.toLowerCase())
		rimraf.sync(thisFinalContainer)
		 ...
		 * Right now (since this is still in development) we're only handling the
		 * game's dependencies, and putting them into the index.
		 *
		 * This function will probably end up getting rewritten.
		 */

		fs.mkdirSync(thisFinalContainer)

		//
		// MAGIC STUFF
		//

		const linkSource = createLink(sanitizedPackage, sanitizedFinal.toLowerCase())
		fs.writeFileSync(path.join(dependencyRoot, `${sanitizedFinal}.lua`), linkSource)

 ...
 */
function parsePackage(rotriever: IRotriever): IRotriever {
	let type = rotriever.kayak && rotriever.kayak.type;

	if (!type) type = rotriever.package.content_root ? "package" : "game";

	if (type === "game" && rotriever.package.content_root)
		throw new Error("Package unreadable: Type is fixed to game, but has content root");
	if (type === "package" && rotriever.package.dependencies_root)
		throw new Error("Package unreadable: Type is fixed to package, but has dependencies root");
